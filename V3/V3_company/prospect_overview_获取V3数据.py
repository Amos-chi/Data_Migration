...
def getV3prospectdetails(id_, V3header):
    url = f'https://api-staging.hitalentech.com:8888/company/api/v3/company/prospect/{id_}'
    while True:
        try:
    ...
    address = []
    addressStr = []
    for i in resp.json()['companyAddresses']:
        address.append(i['address'])
        try:
            addressStr.append({
                'city': i['geoInfoEN']['city'],
                'province': i['geoInfoEN']['province'],
                'country': i['geoInfoEN']['country']
            })
        except TypeError:
            pass
    ...
    leadSource = []
    saleLeadOwners = []
    for i in resp.json()['salesLeadDetails']:
        estimatedDealTime.append(i['estimatedDealTime'])
        accountProgress.append(i['accountProgress'])
        leadSource.append(i['leadSource'])
        for s in i['companyServiceTypes']:
            serviceTypeNames.append(s['label'])
        for c in i['salesLeadClientContacts']:
            clientContact.append(c['name'])
        for slo in i['salesLeadsOwner']:
            saleLeadOwners.append(slo['userId'])
    ...
    teamMembers = []
    for t in resp.json()['companyAssignTeamMembers']:
        teamMembers.append(t['fullName'])
...
if __name__ == '__main__':
    ...
    for id_ in compIDs:
        print(f'处理中 : {id_} ..')
        try:
            datas.append(getV3prospectdetails(id_, V3header))
        except Exception as e:
            print(e)
            continue

    f = open('V3_prospect.txt', 'w', encoding='utf-8')
