import requests
# import staging_client_listdata
...
for id in id_staging:
    ...
    # print(original_data)
    original_sals = original_data["salesLeadDetails"]
    if len(original_sals) > 0:
        for sales_ele in original_sals:  ### 获取am
            am = []
            am_original = sales_ele["accountManagers"]
            if am_original !=None:
...
### 获取bd
        bd_original = sales_ele["businessDevelopmentOwner"]
        bd = []
        for j in bd_original:
            bd_ele = {"salesLeadId":j["salesLeadId"],"userId":j["userId"],"contribution":float(j["contribution"])}
            bd.append(bd_ele)
### 获取sales owner
        owners_original = sales_ele["salesLeadsOwner"]
        owner = []
        for owner_yuansu in owners_original:
            owner_ele = {"salesLeadId":owner_yuansu["salesLeadId"],"userId":owner_yuansu["userId"],"contribution":owner_yuansu["contribution"]}
...
###### 获取primary adress
        ...
        # print(sales_ele)
    company_adress_orginal = original_data["companyAddresses"]
    if len(company_adress_orginal) > 0:
        for adress in company_adress_orginal:
            # print(adress)
            oters_adress = []
            adress_type = adress["companyAddressType"]
            if adress_type == "PRIMARY":
                primary_adress_original = adress
                if primary_adress_original != None  and primary_adress_original["geoInfoEN"] !=None :
                        # print(primary_adress_original)
                    companyAddresses_primary =  {"address":primary_adress_original["address"],"address2":primary_adress_original["address2"],"geoInfoEN":{"cityId":primary_adress_original["geoInfoEN"]["cityId"],"city":primary_adress_original["geoInfoEN"]["city"],"province":primary_adress_original["geoInfoEN"]["province"],"country":primary_adress_original["geoInfoEN"]["country"]},"companyAddressType":"PRIMARY"}
            else:

                oters = adress

                if oters != None and oters["geoInfoEN"] != None:
                    othes_ele ={"address":oters["address"],"address2":oters["address2"],"geoInfoEN":{"cityId":oters["geoInfoEN"]["cityId"],"city":oters["geoInfoEN"]["city"],"province":oters["geoInfoEN"]["province"],"country":oters["geoInfoEN"]["country"]},"companyAddressType":"OTHER"}
                    oters_adress.append(othes_ele)
    else:
        companyAddresses_primary = {
		'address': None,
		'address2': None,
	...
	"companyClientLevelType": original_data["companyClientLevelType"],
...
f = open('client_overview_V3_jsonlist.py','w', encoding='utf-8')
